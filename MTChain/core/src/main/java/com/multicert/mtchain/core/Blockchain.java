package com.multicert.mtchain.core;

import com.multicert.mtchain.core.utils.Logs;
import com.multicert.mtchain.core.utils.Utils;
...
public class Blockchain implements Serializable {
    ...
    public String queryDocument(HFClient client, Channel channel, String originalHash){

        try {
            String str = new String();
            if (IDHashtable.containsKey(originalHash)) {
                List<UUID> idList = IDHashtable.get(originalHash);

                for (UUID id : idList) {
                    str = str + op_query(client, channel, originalHash, id) + "\n";
                }
                return str;
            } else {
    ...
    public Hashtable<String, List<UUID>> getIDHashtable(){
        return IDHashtable;
    }
     ...
     *
     ******************************************************************/
    public int start_op(HFClient client, Channel channel, String signer, String hash, String originalHash) throws Exception{


        UUID id = Utils.generateID(IDHashtable);
        Logs.write("Starting signature cycle...");
        ...
        transactionRequest.setArgs(id.toString(), signer, hash, new SimpleDateFormat("yyyy/MM/dd HH:mm").format(new Date()));

        Logs.write("Getting chaincode " + ccId.getName() + ":" + ccId.getVersion());

        Logs.write("Sending op_start proposal to all peers...");
        ...
        idList.add(id);
        IDHashtable.put(originalHash, idList);
        //Utils.serialize(IDHashtable, "IDHashtable:" + name);
        return 0;

    }

    public int sign_op(HFClient client, Channel channel, String signer, String hash, String originalHash) throws Exception{

        UUID id = Utils.generateID(IDHashtable);
        Logs.write("User " + signer +" signing...");
        ...
        Logs.write("Getting chaincode " + ccId.getName() + ":" + ccId.getVersion());
        Logs.write("Sending op_sign proposal to all peers...");
    ...
    public int end_op(HFClient client, Channel channel, String signer, String hash, String originalHash) throws Exception{

        UUID id = Utils.generateID(IDHashtable);
        Logs.write("Closing document signature...");
        ...
        Logs.write("Getting chaincode " + ccId.getName() + ":" + ccId.getVersion());
        Logs.write("Sending op_end proposal to all peers...");
    ...
    private String op_query(HFClient client, Channel channel, String originalHash, UUID id) throws Exception{

        Logs.write("Query blockchain for document with hash " + originalHash);
